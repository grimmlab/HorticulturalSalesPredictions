...
from training import TrainHelper, ModelsANN


def run_ann_optim(target_column: str, split_perc: float, imputation: str, featureset: str):
    """
        ...
        try:
            for train, test in train_test_list:
                model = ModelsANN.AnnRegression(
                    ...
                    one_step_ahead=one_step_ahead, n_feature=train.shape[1] - 1, n_hidden=n_hidden,
                    num_hidden_layer=num_hidden_layer, dropout_rate=dropout_rate, batch_size=batch_size,
            ...
            params_dict = {'dataset': dataset.name, 'featureset': featureset,
                           ...
                           'learning_rate': learning_rate, 'min_val_loss_improvement': min_val_loss_improvement,
                           'max_epochs_wo_improvement': max_epochs_wo_improvement, 'n_hidden': n_hidden,
                           'num_hidden_layer': num_hidden_layer, 'one_step_ahead': one_step_ahead}
    ...
    TrainHelper.save_csv_results(doc_results=doc_results, save_dir=base_dir+'OptimResults/',
                                 company_model_desc='ANN', target_column=target_column,
...
if __name__ == '__main__':
    ...
    for (imputation, featureset) in imp_feat_combis:
        new_pid = os.fork()
        if new_pid == 0:
            run_ann_optim(target_column=target_column, split_perc=split_perc, imputation=imputation,
                          featureset=featureset)
            sys.exit()
        else:
